...
- Refracts the **actual DOM inside a source element** through a rounded-rectangle lens, using ordinary CSS `filter` and SVG `feDisplacementMap`.
...
- Provides strength, refractive index, bevel, corner radius, local frost, and edge highlight controls.
...
3. The optical surface is a rounded rectangle. Lens positions are not automatically aligned with unrelated fixed/sticky elements or transformed ancestors.
